...
Sadly, in the age of COVID-19, plans don't always pan out. The pandemic did <i>not</i> wind down, the country did <i>not</i> reopen, and the front was more or less abandoned. You can, however, still view the afore-mentioned animated logo [here](https://www.usatoday.com/rebuilding-america/){:target="_blank"}.
...
{#each paths as path, i}
{#if i == 5 || i == 6}
<path in:draw="{{ delay: i * 200, duration: 1000 }}" opacity="0.5" d={path}/>
{:else}
<path in:draw="{{ delay: i * 200, duration: 1000 }}" d={path}/>
{/if}
{/each}
...
<path in:draw="{{ delay: i * 200, duration: 1000 }}" opacity="0.5" d={path}/>
{:else if i == 10}
<path in:draw="{{ delay: 2500, duration: 1000 }}" on:introend="{() => done = true}" d={path}/>
{:else}
<path in:draw="{{ delay: i * 200, duration: 1000 }}" d={path}/>
{/if}
{/each}
...
<div class="skills-cloud">
  ...
  <span in:fade="{{ delay: Math.random() * 2500 + 1000, duration: 100 }}">{ character }</span>
  {/each}
</div>
{% endraw %}
```
